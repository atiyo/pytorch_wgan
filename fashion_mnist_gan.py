...
def save_output_plot(pytorch_variable, index, path='./output/'):
...
if __name__=='__main__':
    ...
    for epoch in range(num_epochs):
        for step, data in enumerate(image_loader):
            ...
            d_loss = -torch.mean(D(image))
            d_loss.backward()
            d_optimizer.step()

            #train on generated data
            d_optimizer.zero_grad()
            d_loss = torch.mean(D(G(noise)))
            d_loss.backward()
            d_optimizer.step()
            ...
            if step % k == 0 and step > 100:
                ...
                if use_cuda:
                    ...
                    print('Epoch: {}. Step: {}. Generator Loss: {}. Discriminator Loss: {}'.format(epoch, step, g_loss.data.numpy(), d_loss.data.numpy()))
